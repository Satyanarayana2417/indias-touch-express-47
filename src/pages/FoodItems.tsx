...
import { Search, Filter, Star, ShoppingCart, Heart, Leaf, Award, Grid, List, ArrowLeft } from "lucide-react";
...
const FoodItems = () => {
  ...
  // Load food products from Firebase
  useEffect(() => {
    const loadFoodProducts = async () => {
      try {
        setLoading(true);
        // Get products from 'food' category in Firebase
        const foodProducts = await getProductsByCategory('food', 50);
        setProducts(foodProducts);
      } catch (error) {
        console.error('Error loading food products:', error);
        toast({
          title: "Error",
          description: "Failed to load food products. Please try again later.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
    ...
    // Subscribe to real-time updates for food products
    const unsubscribe = subscribeToProducts((allProducts) => {
      // Filter for food category products
  ...
  const categories = [
    { id: "all", name: "All Food Items", count: products.length },
    { id: "spices", name: "Spices & Masala", count: products.filter(p => p.category === 'spices').length },
    { id: "rice-grains", name: "Rice & Grains", count: products.filter(p => p.category === 'rice-grains').length },
    { id: "lentils", name: "Lentils & Pulses", count: products.filter(p => p.category === 'lentils').length },
    { id: "oils-ghee", name: "Oils & Ghee", count: products.filter(p => p.category === 'oils-ghee').length },
    { id: "ready-to-eat", name: "Ready to Eat", count: products.filter(p => p.category === 'ready-to-eat').length },
    { id: "snacks", name: "Snacks & Sweets", count: products.filter(p => p.category === 'snacks').length }
  ];
              ...
              <div className="flex-1">
                ...
                {loading ? (
                  ...
                  <div className="text-center py-12">
                    <p className="text-gray-500 text-lg">No food products found.</p>
                    {searchTerm && (
